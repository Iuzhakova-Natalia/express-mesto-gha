  ...
  Card
    ...
    .catch((err) => {
      if (err.name === 'ValidationError') {
        res.status(ERROR_CODE.BAD_REQUEST).send('Переданы некорректные данные при создании карточки.');
      } else {
        res.status(ERROR_CODE.SERVER_ERROR).send('На сервере произошла ошибка');
      }
    });
};
